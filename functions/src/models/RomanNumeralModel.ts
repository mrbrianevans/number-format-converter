...
const RomanNumeralModel: IModel = {
    ...
    decode: (representation: string) => {
        const legend = "IVXLCDM";
	   	...
	   	{
	      if(!!representation[1] && legend.indexOf(representation[0]) < legend.indexOf(representation[1])){
	         sum += (arr[legend.indexOf(representation[1])] - arr[legend.indexOf(representation[0])]);
	         representation = representation.substring(2, representation.length);
	      } else {
